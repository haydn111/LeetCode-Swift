...
/*
Two theroms to solve this problem:
...
Let i be start postion and j be the offset travels from i. Each round we start from i + j. If gasLeft < 0 then x = i + j and next i = x = i + j.
*/
func canCompleteCircuit(_ gas: [Int], _ cost: [Int]) -> Int {
    var i = 0, j = 0    // i is start position, j is offset travels from i
    while i < gas.count {
        var gasLeft = 0
        j = 0
        while j < gas.count {
